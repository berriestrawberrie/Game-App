...
async function main() {
  ...
  const game1 = await prisma.game.create({
    data: {
      title: "Chess",
      description:
        "A strategic board game where two players battle to checkmate the opponent’s king",
    },
  });
  const game2 = await prisma.game.create({
    data: {
      title: "Chance",
      description:
        "A luck‑driven game where outcomes depend on dice rolls or random draws.",
    },
  });
  const game3 = await prisma.game.create({
    data: {
      title: "Puzzle",
      description:
        "A problem‑solving game that challenges players to arrange, match, or decode pieces into order.",
    },
  });
  const game4 = await prisma.game.create({
    data: {
      title: "Ping",
      description:
        "A classic arcade game simulating table tennis with paddles bouncing a ball back and forth.",
    },
  });
  const now = new Date();
